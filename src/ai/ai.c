...
/**
 * The function of freeing all the parts of the node
*/
...
/**
 * The function to free the whole expanded table
*/
void free_memory(unsigned expanded_nodes, sokoban_t* init_data){
	// Take in the initial data and 
	for( unsigned i = 1; i < expanded_nodes; i++){
		if (expanded_nodes_table[ i ] != NULL){
			freeNode(expanded_nodes_table[ i ], init_data);
		}
...
void find_solution(sokoban_t* init_data, bool show_solution)
{
	...
	while(pq.count > 0) {
		...
		// Check whether the game has win
		...
		for(move_t action = left; action <= down; action++) {
			node_t* child_node;
			bool player_moved = applyAction(init_data, n, &child_node, action);
			generated_nodes++;

			// Check whether the player moved or not 
			if(!player_moved || checkDeadLock(init_data, &(child_node->state))) {
				freeNode(child_node, init_data);
				continue;
			}
